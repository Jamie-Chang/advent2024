...
let mul_expression = Re.(seq [ str "mul("; group (rep1 digit); str ","; group (rep digit); str ")" ])
...
let part1 filename = read_file filename |> operations |> map mul |> sum ;;
...
let parse_group group = 
  let op_string = Re.Group.get group 0 in 
    if String.starts_with ~prefix:"mul" op_string 
      then Mul((Re.Group.get group 1 |> int_of_string), Re.Group.get group 2 |> int_of_string)
      else if op_string = "don't" then Dont else Do


let get_ops text = Re.Seq.all regex text |> of_seq |> map parse_group

let run_ops ops = 
  let run_op (total, state) op = match (state, op) with
    | (_, Do) -> (total, Enabled)
    | (_, Dont) -> (total, Disabled)
    | (Disabled, _) -> (total, Disabled)
    | (Enabled, Mul(x,y)) -> (total + x * y, Enabled) in
    fold run_op (0, Enabled) ops

let part2 filename = read_file filename |> get_ops |> run_ops |> fun (total, _) -> total ;;
